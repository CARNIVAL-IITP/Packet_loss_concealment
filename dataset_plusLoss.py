...
class GEN_REAL_TestLoader(Dataset):
    def __init__(self):
        dataset_name = CONFIG.DATA.dataset
        test_dir = CONFIG.TEST.in_dir
        self.mask = CONFIG.DATA.EVAL.masking

        self.target_root = CONFIG.DATA.data_dir[dataset_name]['root']
        # self.lossy_data_list = glob.glob(os.path.join(test_dir, '*.wav'))
        # self.lossy_data_list = self.lossy_data_list.sort()
        txt_list = CONFIG.DATA.data_dir[dataset_name]['test']
    ...
    def __len__(self):
        return len(self.lossy_data_list)

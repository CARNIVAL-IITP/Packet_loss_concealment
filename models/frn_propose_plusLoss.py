...
from models.blocks_propose_plusLoss import Encoder, Predictor, RI_Predictor, AcousticLoss
...
class PLCModel(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        # print('output', output.shape) # output torch.Size([32, 2, 160, 257]) B, R/I, F, T
    ...
    def training_step(self, batch, batch_idx):
        ...
        ##### add for TAPLoss: start
        x_TAP = x
        y_TAP = y
        x_TAP = torch.view_as_complex(x_TAP.permute(0, 2, 3, 1).contiguous())
        x_TAP = torch.istft(x_TAP, self.window_size, self.hop_size, window=self.window)
        y_TAP = torch.view_as_complex(y_TAP.permute(0, 2, 3, 1).contiguous())
        y_TAP = torch.istft(y_TAP, self.window_size, self.hop_size, window=self.window)
        # print('check',x.shape, y.shape) #check torch.Size([32, 40960]) torch.Size([32, 40960])
        TAPLoss = AcousticLoss('frame_energy_weighted_l1','/home/dh/Workspace/TAPLoss/TAPLoss/WSJ_mine/MyTAP_estimator_model_v107_ckp.pt', False, None,
                               device='cuda') # cuda: train, cpu: eval
        TAP_loss = TAPLoss(y_TAP, x_TAP, 'train') # clean, enh, mode (train or eval)
        loss = self.loss(x,y) + 0.3*TAP_loss
        # print('new loss', loss, self.loss(x,y), TAP_loss) #  tensor(0.3732) tensor(0.2467) tensor(0.1266)
        # exit()
        #### add for TAPLoss: end
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        # loss = self.loss(pred, y) # FRN 오리지널
        FRN_loss = self.loss(pred, y)  # FRN 오리지널
        ...
        TAPLoss = AcousticLoss('frame_energy_weighted_l1','/home/dh/Workspace/TAPLoss/TAPLoss/WSJ_mine/MyTAP_estimator_model_v107_ckp.pt', False, None,
                               device='cuda') # cuda: train, cpu: eval
        TAP_loss = TAPLoss(y, pred, 'train') # clean, enh, mode (train or eval)
        loss = FRN_loss + 0.3*TAP_loss
        #### add for TAPLoss: end
